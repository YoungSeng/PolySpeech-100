...
# NOTE: This script MUST be run from inside the cloned `covoaudio` repository
# so that the following local modules can be successfully imported.
from transformers import AutoConfig, AutoModel
from transformers.models.qwen2 import Qwen2TokenizerFast
from safetensors.torch import load_file
from covoaudio.configuration_covo_audio import CovoAudioConfig
from covoaudio.modeling_covo_audio import CovoAudioForCausalLM, get_dialog_prompt
...
def get_config():
    parser = argparse.ArgumentParser(description="PolySpeech-100 Evaluation for CovoAudio (Audio-Only)")

    # 1. Core Experiment Settings
    parser.add_argument("--lang", type=str, default="all", help="Specify language, e.g., 'eng_Latn', or 'all'")
    ...
    parser.add_argument("--output_path", type=str, default="../results/Covo_audio/",
                        help="Directory to output results")
    parser.add_argument("--model_dir", type=str, default="./covoaudio",
                        help="Path to the downloaded CovoAudio model weights")
...
INSTRUCTION_FILE = os.path.join(ASSETS_DIR, "Instruction.mp3")
...
def stitch_audio_files(folder_path, output_filename):
    ...
    target_sr = 48000 # CovoAudio specific sample rate
    data_list = []
    ...
    if os.path.exists(INSTRUCTION_FILE):
        add_audio(INSTRUCTION_FILE)
        add_silence()
    else:
        print(f"Warning: Instruction file not found at {INSTRUCTION_FILE}")
    ...
    audio_duration = len(final_audio) / target_sr
...
def augment_audio(audio_data, mode='clean', sr=48000):
...
def run_evaluation(model, tokenizer, eos_token_id, current_lang, global_start_time, global_counter, total_global_tasks):
    ...
    if N_SHOTS > 0:
        ...
        for shot_name in shot_folders:
            ...
            shot_instruction = f"{SYSTEM_PROMPT}\nPlease answer the question based on the audio."

            history_messages.append({
                "role": "user",
                "content": [
                    {"type": "audio", "audio_url": shot_wav_path},
                    {"type": "text", "text": shot_instruction}
                ]
            })
            history_messages.append({
                "role": "assistant",
            ...
            print(f"   -> Added shot: {shot_name} | Ans: {shot_gt}")
    ...
    for idx, folder_name in enumerate(final_todo_list):
        ...
        try:
            # Feature extraction for CovoAudio
            ...
            # Model Generation (Greedy decoding for multiple choice)
            ...
            cleaned_text = tokenizer.decode(filtered, skip_special_tokens=True).strip()
...
if __name__ == '__main__':
    ...
    print("Loading CovoAudio model...")
    config = CovoAudioConfig.from_pretrained(MODEL_DIR)
    model = AutoModel.from_config(config).to("cuda:0").eval()
    ...
    for i, lang in enumerate(target_languages):
        run_evaluation(model, tokenizer, eos_token_id, lang, global_start_time, global_counter, total_global_tasks)
